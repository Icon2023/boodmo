...
const Review = () => {
    ...
    const handleSubmit = () => {
        let data = {
            currency: "INR",
            amount: parseInt(countTotal(login_cart) - ((countTotal(login_cart)) * (coupon_code?.coupon_discount / 100)) ? countTotal(login_cart) - ((countTotal(login_cart)) * (coupon_code?.coupon_discount / 100)).toFixed(2) : countTotal(login_cart)) * 100
        }
        console.log(data);
